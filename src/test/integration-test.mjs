 ...
 *
 * 运行: node packages/agent-browser-agent/dist/test/integration-test.mjs
 * 或者: node --experimental-strip-types packages/agent-browser-agent/src/test/integration-test.ts
 */

import { createServer } from 'http';
import { AgentBrowserAgent } from '../../dist/index.mjs';

const sdkModule = await import('../../../agent-browser-sdk/dist/index.mjs');
const AgentBrowserSdk = sdkModule.AgentBrowserSdk;
...
function createMockRabbitAIServer(validToken, port) {
...
async function testSingleExecute(sdk) {
...
async function testSingleExecuteError(sdk) {
...
async function testBatchExecute(sdk) {
...
async function testUnknownEngine(sdk) {
...
async function testDisconnect(sdk) {
  console.log('\n=== Test 7: Disconnect ===');
...
async function testAgentStop(agent) {
  console.log('\n=== Test 8: Agent stop ===');
